...
from app.analytics.hyperliquid_perp_chat import (
    HYPERLIQUID_PERP_CHAT_QUICK_PROMPTS,
    HyperliquidPerpChatResponse,
    HyperliquidPerpChatSession,
    create_hyperliquid_perp_chat_session,
    hyperliquid_perp_chat_transcript_path,
    normalize_hyperliquid_perp_coin,
    redact_hyperliquid_perp_chat_secrets,
    save_hyperliquid_perp_chat_transcript,
)
from app.ui import polished_theme


def open_hyperliquid_perp_chat_window(
    parent: tk.Misc,
    coin: str = "HYPE",
    *,
    app_context: object | None = None,
    session_factory: Callable[[str], HyperliquidPerpChatSession] | None = None,
) -> "HyperliquidPerpChatWindow":
    window = HyperliquidPerpChatWindow(
        parent,
        coin,
...
class HyperliquidPerpChatWindow(tk.Toplevel):
    def __init__(
        self,
        parent: tk.Misc,
        coin: str = "HYPE",
        *,
        ...
        self.coin = normalize_hyperliquid_perp_coin(coin or "HYPE")
        self.app_context = app_context
        self.session_factory = session_factory or self._default_session_factory
        self.session: HyperliquidPerpChatSession | None = None
        ...
        self.title(f"Hyperliquid Perp Strategy Chat - {self.coin}")
        ...
        self.status_var = tk.StringVar(value=f"Loading {self.coin} perp context...")
        self.header_title_var = tk.StringVar(value=f"Hyperliquid Perp Strategy Chat - {self.coin}")
        self._build_header()
        self._build_transcript()
        self._build_prompt_area()
        self._set_controls_enabled(False)
        self._append_system_line(f"Loading {self.coin} deterministic perp context...")
    ...
    def _build_header(self) -> None:
        ...
        ttk.Label(
            header,
            text="Read-only strategy chat. Orders must be submitted manually.",
            style="Subtle.TLabel",
        ...
        quick_actions.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(8, 0))
        self.quick_buttons: list[ttk.Button] = []
        for label, prompt in HYPERLIQUID_PERP_CHAT_QUICK_PROMPTS.items():
            button = ttk.Button(quick_actions, text=label, command=lambda value=prompt: self._send_prompt(value))
            button.pack(side=tk.LEFT, padx=(8, 0), pady=(0, 4))
    ...
    def _default_session_factory(self, coin: str) -> HyperliquidPerpChatSession:
        return create_hyperliquid_perp_chat_session(
            coin,
    ...
    def _load_session_in_background(self) -> None:
        ...
        def worker() -> None:
            try:
                session = self.session_factory(self.coin)
    ...
    def _finish_session_loaded(self, session: HyperliquidPerpChatSession) -> None:
        if self._closed:
            return
        ...
        self.coin = session.context.selected_coin
        self.title(f"Hyperliquid Perp Strategy Chat - {self.coin}")
        self.header_title_var.set(session.context.display_name)
        metadata = session.context.source_metadata
        available_count = len(metadata.get("available", []) or [])
        unavailable_count = len(metadata.get("unavailable", []) or [])
        warning_count = len(metadata.get("warnings", []) or [])
        status = f"Ready - {available_count} sources loaded, {unavailable_count} limited"
        if warning_count:
            status += f", {warning_count} warning(s)"
        self.status_var.set(status + ".")
        self._set_controls_enabled(True)
        self._append_system_line(
            f"{self.coin} context loaded. Analysis uses deterministic perp math and synced cockpit context only."
        )
    ...
    def _finish_session_error(self, error: Exception) -> None:
        if self._closed:
            return
        self._context_loading = False
        self.status_var.set("Hyperliquid perp chat could not load context.")
        self._append_system_line(f"Context load failed: {error}")
        self.refresh_button.configure(state=tk.NORMAL)
        messagebox.showerror("Hyperliquid Perp Strategy Chat", str(error))

    def _refresh_context(self) -> None:
        if self._request_running:
            self.status_var.set("Wait for the current strategy chat response to finish.")
            return
        self.status_var.set(f"Refreshing {self.coin} perp context...")
        self._append_system_line(f"Refreshing {self.coin} deterministic perp context...")
        self._load_session_in_background()
    ...
    def _send_prompt(self, prompt: str) -> None:
        if self.session is None:
            self.status_var.set("Still loading Hyperliquid perp context...")
            return
        if self._request_running:
            self.status_var.set("Wait for the current strategy chat response to finish.")
            return
        ...
        self.status_var.set("Preparing Hyperliquid perp context...")
    ...
    def _finish_prompt_success(self, response: HyperliquidPerpChatResponse, request_generation: int | None = None) -> None:
    ...
    def _finish_prompt_error(self, error: Exception, request_generation: int | None = None) -> None:
        ...
        if was_cancelled:
            ...
            return
        self.status_var.set("OpenAI Hyperliquid perp chat failed.")
        self._append_system_line(f"OpenAI request failed: {error}")
        messagebox.showerror("Hyperliquid Perp Strategy Chat failed", str(error))
    ...
    def _append_message(self, role: str, content: str) -> None:
        label = "You" if role == "user" else "Perp Strategy Analyst"
        tag = "user" if role == "user" else "assistant"
        self.transcript.configure(state=tk.NORMAL)
        self.transcript.insert(tk.END, f"\n{label}\n", tag)
        self.transcript.insert(tk.END, redact_hyperliquid_perp_chat_secrets(content.strip()) + "\n")
    ...
    def _save_transcript(self) -> None:
        if self.session is None:
            messagebox.showinfo("Save Transcript", "Hyperliquid perp context is still loading.")
            return
        default_path = hyperliquid_perp_chat_transcript_path(self.session.context.selected_coin)
        default_path.parent.mkdir(parents=True, exist_ok=True)
        selected = filedialog.asksaveasfilename(
            parent=self,
            title="Save Hyperliquid perp strategy chat transcript",
            defaultextension=".md",
        ...
        if not selected:
            return
        try:
            path = save_hyperliquid_perp_chat_transcript(self.session, Path(selected))
        except Exception as exc:
