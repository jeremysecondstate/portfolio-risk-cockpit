...
def _ensure_execution_workspace_vars(self: tk.Tk) -> None:
    ...
    _ensure_string_var(self, "hyperliquid_quote_asset_var", "USDC")
    ...
    _ensure_string_var(self, "hyperliquid_spot_quote_asset_var", getattr(self, "hyperliquid_quote_asset_var", tk.StringVar(value="USDC")).get())
...
def _sync_hyperliquid_ticket_from_shared(self: tk.Tk, ticket_kind: str) -> None:
    if ticket_kind == "spot":
        ...
        self.hyperliquid_spot_quote_asset_var.set(_selected_hyperliquid_workspace_spot_quote(self))
...
def _on_hyperliquid_spot_quote_changed(self: tk.Tk) -> None:
    quote = _selected_hyperliquid_workspace_spot_quote(self)
    raw_market = self.hyperliquid_spot_symbol_var.get().strip()
    if raw_market and not raw_market.startswith("@"):
        base = raw_market.split("/", 1)[0].strip()
        if base:
            self.hyperliquid_spot_symbol_var.set(f"{base}/{quote}")
    if self.hyperliquid_spot_coin_var.get().strip().startswith("@"):
        self.hyperliquid_spot_coin_var.set("")
    _sync_hyperliquid_workspace_spot_size_unit(self)


def _hyperliquid_workspace_spot_market_with_quote(self: tk.Tk) -> str:
...
def _build_hyperliquid_trading_tab(self: tk.Tk, parent: ttk.Frame) -> None:
    ...
    def hyperliquid_account_action(ticket_kind: str, account_key: str, *names: str) -> Callable[[], None]:
        return lambda: _run_hyperliquid_ticket_action(
            self,
            ticket_kind=ticket_kind,
            preview_widget=self.hyperliquid_trading_preview_text,
            command=lambda: _first_available_command(self, *names)(account_key),
        )
    ...
    _add_workspace_button(actions, row=3, column=0, text="LIVE Submit Jeremy", command=hyperliquid_account_action("perp", "jeremy", "show_hyperliquid_perp_live_submit_for_account"), style="Danger.TButton", columnspan=3,)
    _add_workspace_button(actions, row=4, column=0, text="LIVE Submit Alex", command=hyperliquid_account_action("perp", "alex", "show_hyperliquid_perp_live_submit_for_account"), style="Danger.TButton", columnspan=3,)
