...
def _install_option_chain_widgets(self: tk.Tk) -> None:
    ...
    ticket = _find_labelframe(self, "Schwab Stock / ETF Ticket")
    if ticket is None:
        return

    chain = ttk.LabelFrame(ticket, text="Schwab Option Chain", style="Card.TLabelframe")
    chain.grid(row=3, column=0, sticky="nsew", pady=(10, 0))
    chain.columnconfigure(0, weight=1)
    chain.rowconfigure(1, weight=1)

    controls = ttk.Frame(chain, style="Panel.TFrame")
    controls.grid(row=0, column=0, sticky="ew", pady=(0, 6))
    controls.columnconfigure((0, 1, 2), weight=1, uniform="option_chain_buttons")
    ttk.Button(
        controls,
    ...
    tree = ttk.Treeview(table_frame, columns=_OPTION_CHAIN_COLUMNS, show="headings", height=8)
    headings = {
    ...
    tree.bind("<Double-1>", lambda _event: _use_selected_schwab_option(self, self.options_type_var.get().strip().lower() or "call"))
