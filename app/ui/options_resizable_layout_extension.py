...
def install_options_resizable_layout_extension() -> None:
    """Give the Schwab options what-if workspace clean draggable panes."""
...
def _skip_bottom_options_loader(app: tk.Tk, parent: ttk.Frame) -> None:
    """The compact options lab already has one Schwab technicals button."""
    return


def _build_resizable_options_lab_tab(app: tk.Tk, parent: ttk.Frame) -> None:
    """Build the options lab with a simple Hyperliquid-style split layout."""
    ...
    body.add(left_shell, minsize=520, stretch="always")
    body.add(right_shell, minsize=520, stretch="always")
    app.after_idle(lambda: _safe_sash_place(body, 0, max(650, int(parent.winfo_width() * 0.52)), 0))
...
def _build_options_lab_header(app: tk.Tk, parent: ttk.Frame) -> None:
    ...
    ttk.Button(
        actions,
        text="Run What-If",
        command=lambda: options_lab.run_options_what_if(app),
        style="Accent.TButton",
    ).pack(side=tk.LEFT)
    ttk.Button(
        actions,
        text="Load Schwab Technicals",
        command=lambda: _load_schwab_technicals(app),
        style="Accent.TButton",
    ).pack(side=tk.LEFT, padx=(8, 0))


def _load_schwab_technicals(app: tk.Tk) -> None:
    command = getattr(app, "load_options_lab_technical_context", None)
    if callable(command):
        command()
    else:
...
def _build_resizable_scenario_builder(app: tk.Tk, parent: ttk.Frame) -> None:
    ...
    stack.add(ticket_shell, minsize=420, stretch="always")
    stack.add(technical_shell, minsize=120, stretch="always")
    ...
    options_lab._grid_pair(ticket, 0, "Symbol", ttk.Entry(ticket, textvariable=app.options_symbol_var), "Underlying", ttk.Entry(ticket, textvariable=app.options_underlying_price_var))
    options_lab._grid_pair(ticket, 1, "Action", ttk.Combobox(ticket, textvariable=app.options_action_var, values=options_lab.ACTIONS, state="readonly"), "Strategy", ttk.Combobox(ticket, textvariable=app.options_strategy_var, values=options_lab.STRATEGIES, state="readonly"))
    options_lab._grid_pair(ticket, 2, "Contracts", ttk.Entry(ticket, textvariable=app.options_contracts_var), "Expiration", ttk.Entry(ticket, textvariable=app.options_expiration_var))
    options_lab._grid_pair(ticket, 3, "Strike", ttk.Entry(ticket, textvariable=app.options_strike_var), "Call / Put", ttk.Combobox(ticket, textvariable=app.options_type_var, values=options_lab.OPTION_TYPES, state="readonly"))
    options_lab._grid_pair(ticket, 4, "Bid", ttk.Entry(ticket, textvariable=app.options_bid_var), "Ask", ttk.Entry(ticket, textvariable=app.options_ask_var))
    options_lab._grid_pair(ticket, 5, "Mark", ttk.Entry(ticket, textvariable=app.options_mark_var), "Limit / Debit", ttk.Entry(ticket, textvariable=app.options_premium_var))
    options_lab._grid_pair(ticket, 6, "Order type", ttk.Combobox(ticket, textvariable=app.options_order_type_var, values=options_lab.ORDER_TYPES, state="readonly"), "Time in force", ttk.Combobox(ticket, textvariable=app.options_tif_var, values=options_lab.TIME_IN_FORCE, state="readonly"))
    options_lab._grid_pair(ticket, 7, "Short strike", ttk.Entry(ticket, textvariable=app.options_short_strike_var), "Credit", ttk.Entry(ticket, textvariable=app.options_credit_var))
    options_lab._grid_pair(ticket, 8, "Shares", ttk.Entry(ticket, textvariable=app.options_quantity_var), "Stop price", ttk.Entry(ticket, textvariable=app.options_stop_price_var))
    options_lab._grid_pair(ticket, 9, "Target price", ttk.Entry(ticket, textvariable=app.options_target_price_var), "ATR %", ttk.Entry(ticket, textvariable=app.options_atr_var))

    technical = ttk.LabelFrame(technical_shell, text="Technical Context", style="Card.TLabelframe")
...
def _place_left_stack_sashes(stack: tk.PanedWindow, parent: ttk.Frame) -> None:
    ...
    ticket_height = max(430, int(height * 0.74))
    _safe_sash_place(stack, 0, 0, ticket_height)
...
def _build_resizable_options_output(app: tk.Tk, parent: ttk.Frame) -> None:
    ...
    stack.add(metrics_shell, minsize=120, stretch="always")
    stack.add(summary_shell, minsize=62, stretch="always")
    stack.add(output_shell, minsize=220, stretch="always")
    app.after_idle(lambda: _place_right_stack_sashes(stack, parent))

    metrics = ttk.LabelFrame(metrics_shell, text="Risk + Margin", style="Card.TLabelframe")
