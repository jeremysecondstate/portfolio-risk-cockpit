...
def _render_openai_ipo_filing_report_markdown(report: IpoFilingReport) -> str:
    if report.ai_markdown_report:
        return _render_openai_markdown_memo(report)
...
def _render_openai_markdown_memo(report: IpoFilingReport) -> str:
    memo = report.ai_markdown_report.strip()
    lines = [
        f"# {report.company_name} {report.form} AI Filing Report",
        "",
        f"Source: [{_source_label(report)}]({report.source_url})",
    ...
    if report.facts_json:
        lines.extend(["", "## Facts metadata", "", "```json", report.facts_json.strip(), "```"])

    lines.extend(
        [
            "",
            "## Parser notes",
            "- This report was generated by OpenAI from the selected SEC prospectus text plus deterministic parser facts.",
            "- The model was instructed to use only the provided filing text and to mark missing or ambiguous values as Not disclosed or Not confidently extracted.",
        ]
    )
    if report.model_name:
    ...
    if report.source_mode:
        lines.append(f"- AI source mode: {report.source_mode}")
    if report.source_section_names:
        lines.append(f"- Source sections sent to model: {', '.join(report.source_section_names)}")
    if report.source_section_debug:
        lines.append("- Source section debug:")
